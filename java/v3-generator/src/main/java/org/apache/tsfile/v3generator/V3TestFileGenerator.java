...
package org.apache.tsfile.v3generator;
...
/**
 * Generates V3 format test files for C# interoperability testing.
 * Uses TSFile 1.1.3 API.
 */
public class V3TestFileGenerator {
    ...
    private static void generateSimpleV3Files(String outputDir) throws IOException, WriteProcessException {
    ...
    private static void generateV3File(
        ...
        try (TsFileWriter writer = new TsFileWriter(file)) {
            // Register schema using Path
            MeasurementSchema schema =
                    new MeasurementSchema(SENSOR, dataType, encoding, CompressionType.UNCOMPRESSED);
            Path path = new Path(DEVICE);
            writer.registerTimeseries(path, Collections.singletonList(schema));
